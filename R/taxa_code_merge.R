...
merge_usdaplants_taxa <- function(df, 
        df_usdacol,
        crossref_path="",
        df_keepcol=NULL, # An additional set of colums to retain in the output
				crossref_name="usda-plants 8-1-2023.csv",
				crossref_usdacol="Symbol",
				crossref_codecol="Symbol"){
  # Load the JRN LTER code crossref file. This file lists all known LTER 
  # codes and links them to a species binomial and taxonomic authority code
  crossref <- read_csv(paste(crossref_path, crossref_name, sep='/'))
  ...
  df_edit <- df %>% #select(-path, -habit, -form) %>%
    # There is standard evaluation error with using variables for column names
    # See: https://stackoverflow.com/questions/28125816/r-standard-evalation-for-join-dplyr
    left_join(crossref, by = setNames(crossref_usdacol, df_usdacol))
